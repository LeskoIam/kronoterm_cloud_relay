...
def test_about():
    ...
    AND the response JSON should contain 'detail' as 'kronoterm-cloud-relay' and 'version' as '0.0.23'

    ## Note: The version number used for test is automatically updated in the code at build time.
    """
    response = client.get("/")
    assert response.status_code == 200
    assert response.json() == {"detail": "kronoterm-cloud-relay", "version": "0.0.23"}
